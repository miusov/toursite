...
echo '<button id="bt">VIEW</button>';
	...
	$sel='SELECT co.country, ci.city, ho.hotel, ho.cost, ho.stars FROM hotels ho, cities ci, countries co WHERE ho.cityid=ci.id AND ho.countryid=co.id';
	$res=mysql_query($sel);
	...
	while ($row=mysql_fetch_array($res,MYSQL_NUM)) {
			echo '<tr id="'.$row[1].'">';
			echo '<td>'.$row[0].'</td><td>'.$row[1].'</td><td>'.$row[2].'</td><td>'.$row[3].'</td><td>'.$row[4].'</td><td><a href="http://google.com" target="_blanc">more info</a></td>';
			echo '</tr>';
		}
